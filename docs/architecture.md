...
Orders follow the same layering: Domain (aggregates, status transitions, Raise-only events) → Application (`ICheckoutService`, `IOrderCancellationService`, `ICustomerOrderService`, `ISellerOrderService`, `IOrderNotificationService`, `IProductInventory`) → Infrastructure (EF) → Api (controllers). Domain Orders do not reference EF, ASP.NET, Hangfire, SignalR, notification infrastructure, or Catalog. `Order.Cancel()` is status-only (`Placed → Cancelled`). Product and ProductVariant own stock mutation rules. Application orchestrates checkout decrement and Placed-cancel increment through `IProductInventory`; one `SaveChangesAsync` persists Order + stock. Infrastructure uses existing EF/Npgsql `xmin`; there is no new transaction abstraction. API cancel routes are unchanged; `OrderResponse` has no stock fields. Lifecycle notifications go through `IOrderNotificationService` → `INotificationPublisher` after successful `SaveChangesAsync`. Sprint 9 did not introduce MediatR, event bus, generic repository, `IUnitOfWork`, `BeginTransaction`, reservation tables, a stock ledger, or payment changes.
